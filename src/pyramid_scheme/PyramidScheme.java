...
ArrayList<MultiTreeNode<Person>> nodeChain = new ArrayList<>();

    // Base case if the current node contains the target, add it to the chain
    if (node.getElement().equals(target)) {
        nodeChain.add(node);
    } else {
        // Recursivly check each child node
        for (MultiTreeNode<Person> child : node.getChildren()) {
            // Recursively call findNodeChain on each child
            ArrayList<MultiTreeNode<Person>> childChain = findNodeChain(child, target);

            // If the target is found in the child's chain, add the current node to the chain
            if (!childChain.isEmpty()) {
                nodeChain.add(node);
                nodeChain.addAll(childChain);
                break;  // No need to continue searching other children
            }
        }
    }
